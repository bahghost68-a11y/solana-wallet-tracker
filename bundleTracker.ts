...
interface SharedResources {
  config: any;
  wsSend: (payload: any) => Promise<number>;
...
const recentBundles: BundleAnalysis[] = [];
...
function init(resources: SharedResources): void {
  shared = resources;
...
function log(msg: string): void {
  ...
  if (shared?.log) shared.log(`[BundleTracker] ${msg}`);
  else console.log(`${prefix} ${msg}`);
}

function logWarn(msg: string): void {
  if (shared?.logWarn) shared.logWarn(`[BundleTracker] ${msg}`);
  else console.warn(`[BundleTracker] ⚠ ${msg}`);
}
...
function cleanCache(): void {
  ...
  for (const [sig, ts] of processedSignatures) {
    if (now - ts > CACHE_TTL) processedSignatures.delete(sig);
  }
...
async function rateLimitedRpcCall(method: string, params: any[]): Promise<any> {
  try {
...
async function subscribeToPumpFun(): Promise<void> {
  if (!shared?.wsSend) {
...
async function handleLogNotification(signature: string, logs: string[]): Promise<void> {
...
async function analyzeForJitoBundle(creationSig: string): Promise<BundleAnalysis | null> {
  ...
  const txSignatures: string[] = block.transactions.map((t: any) => {
  ...
  for (
    let i = creationIndex + 1;
    i < Math.min(creationIndex + 16, block.transactions.length);
    i++
  ) {
    const blockTx = block.transactions[i];
    if (!blockTx || !blockTx.meta || blockTx.meta.err) continue;
    ...
    const involvesToken = (txMeta.postTokenBalances || []).some(
      (tb: any) => tb.mint === tokenMint
    );
    ...
    if (tipResult.tipLamports > 0) {
      ...
      if (!jitoTipAccount) jitoTipAccount = tipResult.tipAccount;
    }
    ...
    if (involvesToken) {
      ...
      for (const tb of txMeta.postTokenBalances || []) {
        if (tb.mint === tokenMint) {
          ...
          const preTb = preBalances.find(
            (p: any) => p.mint === tokenMint && p.accountIndex === tb.accountIndex
          );
    ...
    if (!involvesToken && tipResult.tipLamports === 0) {
      ...
      if (nextTx && nextTx.meta && !nextTx.meta.err) {
        const nextInvolvesToken = (nextTx.meta.postTokenBalances || []).some(
          (tb: any) => tb.mint === tokenMint
        );
        if (!nextInvolvesToken) break;
      } else {
        break;
      }
    }
...
function detectJitoTip(blockTx: any): { tipLamports: number; tipAccount: string } {
  if (!blockTx || !blockTx.meta) return { tipLamports: 0, tipAccount: "" };

  const txMeta = blockTx.meta;
  const txMsg = blockTx.transaction?.message;
  if (!txMsg) return { tipLamports: 0, tipAccount: "" };

  const accountKeys = (txMsg.accountKeys || []).map((k: any) =>
    typeof k === "string" ? k : k.pubkey
  );
  ...
  const allInstructions = [
    ...(txMsg.instructions || []),
    ...(txMeta.innerInstructions || []).flatMap((ii: any) => ii.instructions || []),
  ];
...
function outputResult(analysis: BundleAnalysis): void {
  const now = new Date().toISOString();

  console.log(`\n${C.BOLD}${"═".repeat(70)}${C.RESET}`);
  console.log(`${C.CYAN}${C.BOLD}⏰ ${now}${C.RESET}`);
  console.log(
    `${C.BOLD}🪙 Token Mint: ${C.YELLOW}${analysis.tokenMint}${C.RESET}`
  );
  console.log(
    `${C.DIM}👤 Créateur: ${analysis.creator}${C.RESET}`
  );
  ...
  if (analysis.isBundle) {
    ...
    console.log(
      `${C.RED}${C.BOLD}   📦 Achats bundlés: ${analysis.bundledBuys}${C.RESET}`
    );
    if (analysis.supplyPercentage > 0) {
      console.log(
        `${C.RED}   📊 Supply acheté dans le bundle: ${analysis.supplyPercentage.toFixed(2)}%${C.RESET}`
      );
    }
    console.log(
      `${C.RED}   💰 Jito Tip: ${analysis.jitoTipSOL.toFixed(6)} SOL${C.RESET}`
    );
    console.log(
      `${C.RED}   🏦 Tip Account: ${analysis.jitoTipAccount.slice(0, 20)}...${C.RESET}`
    );
  } else {
    console.log(
    ...
    if (analysis.bundledBuys > 0) {
      console.log(
        `${C.GREEN}   Achats dans le même bloc: ${analysis.bundledBuys} (pas de Jito tip détecté)${C.RESET}`
      );
    }
  }
...
async function sendTelegramAlert(analysis: BundleAnalysis): Promise<void> {
  if (!shared?.sendTelegram) return;
  ...
  switch (command) {
    case "/bundles": {
      ...
      if (bundled.length === 0) {
        if (shared?.sendTelegram) {
          await shared.sendTelegram(
            "📋 Aucun Jito Bundle détecté récemment.",
            chatId
          );
        }
        return true;
      ...
      if (shared?.sendTelegram) await shared.sendTelegram(msg, chatId);
      return true;
    }
    ...
    case "/analyze": {
      ...
      if (!sig) {
        if (shared?.sendTelegram) {
          await shared.sendTelegram(
            "❌ Usage: /analyze <code>&lt;tx_signature&gt;</code>",
            chatId
          );
        }
        return true;
      ...
      try {
        ...
        if (analysis) {
          ...
          if (shared?.sendTelegram) {
            await shared.sendTelegram(
              "❌ Impossible d'analyser cette transaction (pas une création Pump.fun ou TX non trouvée)",
              chatId
            );
          }
        }
